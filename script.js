...
const totalCharacter = document.createElement("li");
const totalMatch = document.createElement('li');
const totalTime = document.createElement('li');
const totalScanWords = document.createElement('li');
const totalReplaced = document.createElement('li');
let sentence;
...
wordSearch.addEventListener('input', function(e) {
  ...
  if (word === '') {
    ...
    if (!isMatch) {
      ...
      preHTML = sentence.replace(pattern, `<span style="color:red;">${word}</span>`)
      preview.innerHTML = preHTML
    }
  }
})
...
button.addEventListener('click', function(e) {
  ...
  let wordScanned = sentence.trim().split(/\s+/).length
...
function lister(title, value, childName) {
  let existingItems = statList.querySelectorAll('li');
  let text = document.createTextNode(`${title}: ${value}`)
  childName.appendChild(text);
  // Check if the item already exists in the list
  for (const item of existingItems) {
     if (item.textContent === `${title}: ${value}`) {
       return;
      }else if(statList.hasChildNodes){
       statList.replaceChildren(childName);
       return;
      }
  }
  ...
  //   }
    ...
    // statList.appendChild(childName)

}


// Stat Listing function
// function listerSecond(title, value, childName) {
//   let existingItems = statList.querySelectorAll('li');
//   let text = document.createTextNode(`${title}: ${value}`)
//   childName.appendChild(text);
//   // Check if the item already exists in the list
